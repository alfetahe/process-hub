defmodule Test.CoordinatorNodeupReconcileTest do
  ...
  synchronous `:sys.get_state/1` issued after a `send/2` observes the message
  already handled, so no polling/sleeps are needed. The positive cross-node merge needs
  ...
  defp state(id), do: :sys.get_state(id)

  test "the configured interval is wired into misc storage; default is 3000" do
    assert Storage.get(
             ProcessHub.Hub.get(start_hub(nodeup_reconcile_interval: 1234)).storage.misc,
             StorageKey.nri()
           ) ===
             1234

    assert Storage.get(ProcessHub.Hub.get(start_hub([])).storage.misc, StorageKey.nri()) ===
             3000
  end
  ...
  test "interval 0 disables the fail-safe — :nodeup schedules no timer" do
    ...
    send(id, {:nodeup, @fake})
    assert state(id).nodeup_reconcile_timers === %{}
  end
  ...
  test ":nodeup schedules a per-node timer that :nodedown cancels" do
    ...
    send(id, {:nodeup, @fake})
    assert Map.has_key?(state(id).nodeup_reconcile_timers, @fake)

    send(id, {:nodedown, @fake})
    refute Map.has_key?(state(id).nodeup_reconcile_timers, @fake)
  end
  ...
  test "the fail-safe never batches a node that is not a same-hub peer" do
    ...
    # 500ms debounce default has not elapsed when get_state runs, so a broken
    # guard that batched unconditionally would leave @fake in the batch here.
    send(id, {:nodeup_reconcile, @fake})
    assert state(id).event_batches.cluster_join.nodes === []
  end
end
