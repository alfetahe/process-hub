defmodule ProcessHub.Handler.ClusterUpdate do
      ...
      {redun, redist, cids} =
        ...
        |> Enum.reduce({[], [], []}, fn {cid, cspec, m, nlist1, nlist2}, {redun, redist, cids} ->
          case Enum.member?(nlist1, local_node) do
            true ->
              {[{cid, nlist2, nlist1, []} | redun], redist, [cid | cids]}
  ...
  defmodule NodeDownBatch do
    @moduledoc """
    Handler for batched node down events.
    Processes multiple node failures together to avoid duplicate redistributions.
    """
